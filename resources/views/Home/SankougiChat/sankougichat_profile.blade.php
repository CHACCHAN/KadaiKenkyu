        ...
        <div class="ms-4">
            <div class="text-end">
                @if($sankougi_chat_user->user_id == Auth::id())
                    <div class="text-secondary position-absolute" style="top :0; right: 25%">画像クリックで編集</div>
                    {{-- プロフィール編集ボタン --}}
                ...
                @else
                    {{-- フォローボタン --}}
                    <button id="FollowButton" class="btn @if(isset($sankougi_chat_follow_check->follow_flag)) btn-dark @else btn-primary @endif mt-2 me-2">
                        @if(isset($sankougi_chat_follow_check->follow_flag))
                            フォロー中
                            @php $flag = 1; @endphp
                        @else
                            フォロー
            ...
            <div id="PreviewContent" class="mt-2">{!! nl2br(htmlspecialchars($sankougi_chat_user->content)) !!}</div>
            ...
            <div class="d-flex">
                <a href="{{ route('Home.sankougichat.follow', [
                    'name_id' => $name_id,
                    'type' => 'Follow',
                    ]) }}" class="btn border-0 p-0 mt-1">
                    <div class="d-flex">
                        <div id="FollowCount" class="text-dark">{{ $sankougi_chat_follows->count() }}</div>
                        <div class="text-secondary ms-1">フォロー中</div>
                    </div>
                </a>
                <a href="{{ route('Home.sankougichat.follow', [
                    'name_id' => $name_id,
                    'type' => 'Follower',
                    ]) }}" class="btn border-0 p-0 mt-1 ms-3">
                    <div class="d-flex">
                        <div id="FollowerCount" class="text-dark">{{ $sankougi_chat_followers->count() }}</div>
                        <div class="text-secondary ms-1">フォロワー</div>
                    </div>
                </a>
            </div>
        </div>
        <div class="mt-2">
            <div class="ps-4 border-top">
                {{-- 投稿カード --}}
                @foreach($sankougi_chats as $sankougi_chat)
                    @if($sankougi_chat->chat_user_id == $sankougi_chat_user->chat_user_id)
                        <div class="border-bottom">
                            <div class="card w-100 mx-auto my-3 border-0">
                                <div class="card-header border-0 p-0 bg-light">
                                    @foreach($sankougi_chat_users as $user)
                                        @if($user->chat_user_id == $sankougi_chat->chat_user_id && $user->name_id == $name_id)
                                            <div class="d-flex">
...
<script type="text/javascript">
    var flag = @if($sankougi_chat_user->user_id != Auth::id()){{ $flag }}@endif;
    var FollowCount = {{ $sankougi_chat_followers->count() }};
    var FollowerCount = {{ $sankougi_chat_follows->count() }};
    // フォローボタン
    document.getElementById('FollowButton').onclick = function() {
        ...
        if(!flag) {
            ...
            document.getElementById('FollowerCount').innerHTML = FollowCount += 1;
            URL = '{{ route('Home.sankougichat.follow.store') }}';
            flag = true;
        } else {
            this.innerHTML = 'フォロー';
            ...
            document.getElementById('FollowerCount').innerHTML = FollowCount -= 1;
            URL = '{{ route('Home.sankougichat.follow.delete') }}';
            flag = false;
        }
        // Fetchで送信
        fetch(URL, {
