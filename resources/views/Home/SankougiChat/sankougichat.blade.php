                                                ...
                                                <div class="d-flex">
                                                    ...
                                                    <div class="d-flex mt-2 ms-2">
                                                        ...
                                                        <div class="text-secondary">
                                                            @if(\Carbon\Carbon::now()->format('Y') - $sankougi_chat->created_at->format('Y') != 0)
                                                                {{ '@' . $user->name_id . '・' . \Carbon\Carbon::now()->format('Y') - $sankougi_chat->created_at->format('Y') . '年前' }}
                                                            @elseif(\Carbon\Carbon::now()->format('m') - $sankougi_chat->created_at->format('m') != 0)
                                                                {{ '@' . $user->name_id . '・' . \Carbon\Carbon::now()->format('m') - $sankougi_chat->created_at->format('m') . 'ヶ月前' }}
                                                            @elseif(\Carbon\Carbon::now()->format('d') - $sankougi_chat->created_at->format('d') != 0)
                                                                {{ '@' . $user->name_id . '・' . \Carbon\Carbon::now()->format('d') - $sankougi_chat->created_at->format('d') . '日前' }}
                                                            @elseif(\Carbon\Carbon::now()->format('H') - $sankougi_chat->created_at->format('H') != 0)
                                                                {{ '@' . $user->name_id . '・' . \Carbon\Carbon::now()->format('H') - $sankougi_chat->created_at->format('H') . '時間前' }}
                                                            @elseif(\Carbon\Carbon::now()->format('i') - $sankougi_chat->created_at->format('i') != 0)
                                                                {{ '@' . $user->name_id . '・' . \Carbon\Carbon::now()->format('i') - $sankougi_chat->created_at->format('i') . '分前' }}
                                                            @elseif(\Carbon\Carbon::now()->format('s') - $sankougi_chat->created_at->format('s') != 0)
                                                                {{ '@' . $user->name_id . '・' . \Carbon\Carbon::now()->format('s') - $sankougi_chat->created_at->format('s') . '秒前' }}
                                                            @else
                                                                {{ '@' . $user->name_id . '・1秒前' }}
                                                            @endif
                                                        </div>
                                                    ...
                                                    <div class="col">
                                                        <div class="d-flex">
                                                            <button class="btn border-0 p-0 m-0">
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" id="goodButton-{{ $sankougi_chat->chat_id }}" class="bi bi-hand-thumbs-up" viewBox="0 0 16 16">
                                                            ...
                                                            <div id="goodCount-{{ $sankougi_chat->chat_id }}" class="ms-1">
                                                                {{ $sankougi_chat->good_count }}
                                                            </div>
                                                        </div>
                                                        <script type="text/javascript">
                                                            document.getElementById("goodButton-{{ $sankougi_chat->chat_id }}").addEventListener("click", function () {
                                                                var Point = document.getElementById("goodCount-{{ $sankougi_chat->chat_id }}");
